...
By default, `findflanks` will write to a file named `mgefinder.findflanks.tsv` (The name can be changed with the 
...
You can run the `pairflanks` command as
    ...
    mgefinder pairflanks FLANKSFILE BAMFILE REFERENCE_GENOME
...
By default, `pairflanks` will write to a file named `mgefinder.pairflanks.tsv` (The name can be changed with the 
`--output_file` parameter). The columns of this file are described as follows:
...
in the `pairflanks` file. That is, all lines in the `mgefinder.inferseq_reference.tsv` with the value `20` in the 
...
`findflanks` command in *mgefinder*. You can run this command as follows:
...
This is an experimental command in *mgefinder*, so use at your own risk. This command is used to perform a local
...
sequence assembly software on their machine. This is not included in the default installation of *mgefinder*.
...
The command can then be executed as:
    ...
    mgefinder extendpairs PAIRSFILE BAMFILE
