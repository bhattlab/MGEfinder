...
This user manual is currently no longer relevant to the current version of *MGEfinder* and is in the process of
being updated. However, the *Installation* instructions and *Tutorial* are up-to-date.

# User Manual 
This user manual is designed as a reference for users who have questions about *mgefinder*. It includes details about
...
## Primary *mgefinder* commands
We now describe each of the primary *mgefinder* commands. These commands are at the center of the *mgefinder* workflow,
...
The `find` command takes a BAM file as input. A local alignment algorithm, such as 
...
In the `pair` step, we only pair flanks with each other if they are within 20 bp of 
...
The `find` algorithm works by identifying candidate insertion sites by searching for clipped-end sites in locally 
...
By default, `find` will write to a file named `mgefinder.find.tsv` (The name can be changed with the 
`--output_file` parameter). The columns of this file are described as follows:

1. `flank_id` - An arbitrary identifier used internally by *mgefinder*.
...
this site. If this number is high, it indicates that the detected insertion is most likely a small indel. `find` 
...
in the file generated by running the `pair` command.
...
internally by *mgefinder* in the next step.
...
You can run the `pair` command as
    ...
    mgefinder pair FLANKSFILE BAMFILE REFERENCE_GENOME

Where `FLANKSFILE` is the output of the `find` command, `BAMFILE` is the binary sequence alignment file used as 
input for `find`, and `REFERENCE_GENOME` is the reference genome that `BAMFILE` was aligned to.
...
`--min_alignment_inner_length` used in `find.`
...
The `pair` command uses a variety of techniques to pair flanks with each other. It first filters insertion flanks 
...
By default, `pair` will write to a file named `mgefinder.pair.tsv` (The name can be changed with the 
`--output_file` parameter). The columns of this file are described as follows:

1. `pair_id` - An arbitrary identifier used to identify the candidate insertion. This is an important identifier that
is used to relate the output of the `pair` command to the output of the various `inferseq` commands.
...
directly from the `consensus_softclip_count` of the `find` output file.
5. `softclip_count_4p` - The number of clipped reads at this site that support the 3' insertion flank. This is also 
taken directly from the `consensus_softclip_count` of the `find` output file.
...
this parameter is 1 base pair, but in reality *mgefinder* is not well suited to identify insertions that are smaller than
...
genome is not already bowtie2 indexed, *mgefinder* will index the genome. Next, it will align the individual flanks 
...
By default, `inferseq-reference` will write to a file named `mgefinder.inferseq_reference.tsv` (The name can be changed 
...
in the `pair` file. That is, all lines in the `mgefinder.inferseq_reference.tsv` with the value `20` in the 
`pair_id` column represent an inferred sequence identified by the `inferseq_reference` command that corresponds to the
candidate insertion pair identified in the `pair` file that is labeled with `20` in the `pair_id` column.
...
The `inferseq-assembly` command takes as input the output of the `pair` command, the BAM file of the sequencing 
...
While mgefinder will automatically index the sequence assembly if it is not already indexed, we recommend that you index 
...
By default, `inferseq-assembly` will write to a file named `mgefinder.inferseq_assembly.tsv` (The name can be changed 
...
By default, `inferseq-overlap` will write to a file named `mgefinder.inferseq_overlap.tsv` (The name can be changed 
with the `--output_file` parameter). The columns of this file are the same as those found in the
"`inferseq-reference`: Output file format" section above, but with the following differences:
...
The `inferseq-database` command takes as input the output of the `pair` command, and a FASTA file of the query
sequences of interest.

While mgefinder will automatically index the query sequence database if it is not already indexed, we recommend that you
...
By default, `inferseq-database` will write to a file named `mgefinder.inferseq_database.tsv` (The name can be changed 
with the `--output_file` parameter). The columns of this file are the same as those found in the
"`inferseq-reference`: Output file format" section above, but with the following differences:
...
`find` command in *mgefinder*. You can run this command as follows:
...
This command first removes secondary alignments from the alignment file. It then formats the file for use by *mgefinder*,
...
The `recall` command takes as input the output of the `pair` command, and the original `BAM` file of the reads
aligned to the reference genome. The `pair` output file actually only requires three columns: 
...
Which should be the same as the values used when originally running the `find` command.
By default `min_alignment_quality` is 20 and `min_alignment_inner_length` is 21.

#### `recall`: Description of implementation

Recall works very similarly to the find algorithm, but it does not attempt to reconstruct consensus flanks.
...
The output format is quite similar to the output of the `find` command. Each row is a specific site (note that
...
section "`find`: Output file format" for more details on what each of these columns means.
